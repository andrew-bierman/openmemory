...
- Workers Analytics Engine captures `openmemory.request`,
  ...
  checks in `bun run observability:alerts`.
...
- The scheduled CI `Analytics Engine threshold check` job runs every 15 minutes when Cloudflare
  analytics secrets are configured and fails on request-error, 5xx, rate-limit,
  graph/RAG latency, or async worker failure threshold breaches.
...
- The `Live Production Benchmark` workflow runs a bounded synthetic hosted
  ...
  GitHub Actions artifact for production comparison. It also restores only
  small, current-run JSONL evidence from recent successful benchmark artifacts
  outside the uploaded report directory, then uploads a markdown trend summary
  with latest, average, best, worst, threshold, and run-over-run deltas.
...
- Optional GitHub and Google login provider readiness is visible in
  `/v1/readiness`; hosted social login is disabled for alpha and still requires
  installing real OAuth app client IDs and secrets before re-enabling.
- Hosted GitHub and Google sign-in dogfooding is tracked in
  ...
  the strict gate is disabled until `GITHUB_CLIENT_ID`, `GITHUB_CLIENT_SECRET`,
  `GOOGLE_CLIENT_ID`, and `GOOGLE_CLIENT_SECRET` are installed and sign-in
...
1. Auth hardening
   ...
   - Install GitHub and Google OAuth app secrets, mark both providers required
     in `config/social-oauth-dogfood.json`, update hosted sign-in evidence, and
     run `bun run social-oauth:check` or the strict `Social OAuth Dogfood`
...
5. CI and deployment
   ...
   - Add Cloudflare Notifications or another dedicated Analytics Engine
     threshold destination when broader routing is needed outside GitHub
     Actions.
